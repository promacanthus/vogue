...
一旦定义了消息，就可以在`.proto`文件上运行响应编程语言的`protocol buffers`编译器来生成数据访问类。这些为每个字段提供了简单的访问器（如`name()`和`set_name()`），以及将整个结构序列化/解析为原始字节的方法。例如，如果选择的语言是C++，在上面的例子上运行编译器将生成一个名为`Person`的类。然后，可以在应用程序中使用此类来填充，序列化和检索`Person`的`protocol buffers`消息。写一些这样的代码：
...
但是，`protocol buffers`并不总是比`XML`更好的解决方案。例如，`protocol buffers`不是使用标记对基于文本的文档（例如`HTML`）建模的好方法，**因为无法轻松地将结构与文本交错**。此外，`XML`是人类可读和可编辑的; `protocol buffers`在它原生的格式中不是人类可读和可编辑的。`XML`在某种程度上也是自我描述的。只有拥有消息定义（如`.proto文件`）时，`protocol buffers`才有意义。
...
完成所有设置后，请尝试按照所选语言的[教程](https://developers.google.com/protocol-buffers/docs/tutorials)进行操作 - 这将指导您创建一个使用`protocol buffers`的简单应用程序。
